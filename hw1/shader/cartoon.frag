...
void main(void)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        float diffuseIntensity = u_Kd * max(dot(lightDirection, transformedVertexNormal), 0.0);
        color += floor(diffuseIntensity * numColors) / numColors * fragColor * u_lights[i].color;

        vec3 reflectDirection = reflect(-lightDirection, transformedVertexNormal);
        float specularIntensity = u_Ks * pow(max(dot(reflectDirection, -cameraDirection), 0.0), u_alpha);
        color += floor(specularIntensity * numColors) / numColors * u_lights[i].color;
    }
